...
readUNFCCC_NDC <- function(subtype, subset) {
  ...
  if (grepl("Emissions", subtype, fixed = TRUE)) {
    # Keep the following formulation to enable reading of older files. Side-effect are messages from readxl, that
    # can are then suppressed.
    input <- readxl::read_excel(NDCfile, sheet = "Emissions", skip = 3, na = c("?", ""), progress = FALSE) %>%
      suppressMessages() %>%
      select("ISO_Code" = 2, "Reference_Year" = 7, "BAU_or_Reference_emissions_in_MtCO2e" = 8, "Target_Year" = 9,
