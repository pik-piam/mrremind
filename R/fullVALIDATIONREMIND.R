...
fullVALIDATIONREMIND <- function(rev = 0) {
  ...
  calcOutput(type = "IEA_ETP", aggregate = columnsForAggregation, file = valfile,
  ...
  valpath <- paste0(file.path(getConfig("outputfolder"), valfile))

  h <- read.report(file = valpath, as.list = FALSE)

  # remove IEA ETP Steel and Cement Production on sub-global level
  h[, , "IEA ETP", pmatch = TRUE][, , c(
