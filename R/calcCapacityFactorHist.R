...
calcCapacityFactorHist <- function(){
  ...
  cf_realworld <- new.magpie(getRegions(hist_gen),c(2005,2010,2015),rem_Irena_map$irena,fill=0)
  cf_realworld[,2015,] <- hist_gen[,2015,rem_Irena_map$irena]/(8760*hist_cap[,2015,rem_Irena_map$irena]) 
  #rename
  ...
  hist_gen <- hist_gen[,c(2005,2010,2015),rem_Irena_map$irena]
  getNames(hist_gen) <- rem_Irena_map$rem
  hist_gen[is.na(cf_realworld[,2015,])] <- 0
