...
calcProjectPipelines <- function(subtype) {
  ...
  if (subtype == "CCS") {
    ...
    gdp <- calcOutput("GDP", scenario = "SSP2", aggregate = FALSE)[eu27, 2020, ]
    ...
    # Source 1: IEA Hydropower Special Market Report
    ...
    x <- add_columns(y, addnm = c("y2025"), dim = 2, fill= NA)
    getSets(x)["d2.1"] <- "period"
  ...
  # Nuclear ----
    ...
    # Thresholds basic calculations, status: operational, construction, inactive
    # -> additional calculations in fullTHRESHOLDS.R
    # ASSUMPTION: min_red
    ...
    y <- add_columns(y, addnm = c("y2020"), dim = 2, fill= NA)
    y <- mbind(y, t)
    ...
    # meta data
    unit <- "GW"
    description <- "Nuclear project pipeline from GEM and IAEA PRIS"
    ...
    x <- mbind(x,
               setNames(x[, , "pre-construction"] + x[, , "announced"],
                        "GlobalEnergyMonitor.Cap|Electricity|Coal.planned.GW"),
               # rename to "operational", "construction"
               setNames(x[, , "operating"],
                        "GlobalEnergyMonitor.Cap|Electricity|Coal.operational.GW"))
