...
readBP <- function(subtype) {
  ...
  if (subtype == "Capacity") {
    ...
    y1 <- read_excel(filename, sheet = "Solar Capacity", range = "A4:Z72")
    data_solar <- tidy_data(y1, "Solar")
    y1 <- read_excel(filename, sheet = "Wind Capacity", range = "A4:AA70")
    data_wind <- tidy_data(y1, "Wind")
    y1 <- read_excel(filename, sheet = "Geothermal Capacity", range = "A4:AA43")
  ...
  # Generation data for Nuclear, Hydro, Solar, Wind, Geobiomass, Other Renewables
    ...
    y1 <- read_excel(filename, sheet = "Nuclear Generation - TWh", range = "A3:BE114")
    data_nuclear <- tidy_data(y1, "Nuclear")

    y1 <- read_excel(filename, sheet = "Hydro Generation - TWh", range = "A3:BE114")
    data_hydro <- tidy_data(y1, "Hydro")

    y1 <- read_excel(filename, sheet = "Solar Generation - TWh", range = "A3:BE114")
    data_solar <- tidy_data(y1, "Solar")

    y1 <- read_excel(filename, sheet = "Wind Generation - TWh", range = "A3:BE114")
    data_wind <- tidy_data(y1, "Wind")

    y1 <- read_excel(filename, sheet = "Elec Gen from Gas", range = "A3:AK58")
    data_gas <- tidy_data(y1, "Gas")

    y1 <- read_excel(filename, sheet = "Elec Gen from Oil", range = "A3:AK58")
    data_oil <- tidy_data(y1, "Oil")

    y1 <- read_excel(filename, sheet = "Elec Gen from Coal", range = "A3:AK58")
    data_coal <- tidy_data(y1, "Coal")

    y1 <- read_excel(filename, sheet = "Geo Biomass Other - TWh", range = "A3:BE114")
    ...
    y1 <- read_excel(filename, sheet = "Coal Production - EJ", range = "A3:AO62")
    data_coal_ej <- tidy_data(y1, "Coal_EJ") # in EJ

    y1 <- read_excel(filename, sheet = "Coal Production - Tonnes", range = "A3:AO62")
    data_coal_ton <- tidy_data(y1, "Coal_Ton") # in tonnes

    y1 <- read_excel(filename, sheet = "Gas Production - EJ", range = "A3:AZ78")
    data_gas <- tidy_data(y1, "Gas_EJ") # in EJ

    y1 <- read_excel(filename, sheet = "Oil Production - Tonnes", range = "A3:BE80")
