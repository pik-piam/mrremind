...
readGCPT <- function(subtype) {
  ...
  cap2020 <- cap2020 %>% select_(-~`Announced + Pre-permit + Permitted`)
  ...
  mothballed <- mothballed %>% select_(~`Tracker ID`,~Country,~MW,~Jul20,~Jul19,~Jul18,~Jul17,~Jul16,~Jan15)
  colnames(mothballed)[4:9] <- c(2020:2015)
  mothballed <- filter_(mothballed,(~`2019`=="Moth" | ~`2018`=="Moth" | ~`2017`=="Moth" | ~`2016`=="Moth") & ~`2015`!="Moth" & ~`2020`!="Oper")
  cap_moth <- mothballed %>% group_by_(~Country) %>% summarise_(Cap_Moth=sum(~MW))
  ...
  for (i in 2000:2019) {
    cap_add <- additions %>% select_(~Country,as.character(i):~`2020`)
    cap_add <- mutate_(cap_add,Added_Cap=rowSums(cap_add[,2:ncol(cap_add)]))
    ...
    cap_ret <- retirements %>% select_(~Country,as.character(i):~`2020`)
    cap_ret <- mutate_(cap_ret,Ret_Cap=rowSums(cap_ret[,2:ncol(cap_ret)]))
  ...
  # cap2015_add <- additions %>% select_(~Country,`2016`:`2020`)
  # cap2015_add <- mutate_(cap2015_add,post_2015_add=`2016`+`2017`+`2018`+`2019`+`2020`)
  # cap2015_add$Country <- toolCountry2isocode(cap2015_add$Country)
  # cap2015_add <- as.magpie(cap2015_add,spatial=1)
  # cap2015_add <- toolNAreplace(toolCountryFill(cap2015_add),replaceby = 0)[[1]]
  # 
  # cap2015_ret <- retirements %>% select_(~Country,`2016`:`2020`)
  # cap2015_ret <- mutate_(cap2015_ret,post_2015_ret=`2016`+`2017`+`2018`+`2019`+`2020`)
  # cap2015_ret$Country <- toolCountry2isocode(cap2015_ret$Country)
  # cap2015_ret <- as.magpie(cap2015_ret,spatial=1)
  # cap2015_ret <- toolNAreplace(toolCountryFill(cap2015_ret),replaceby = 0)[[1]]
  # 
  # mothballed <- new.magpie(tran$Country,years=2015,names="Mothballed",fill=tran$moth)
  # getRegions(mothballed) <- toolCountry2isocode(getRegions(mothballed))
  # mothballed <- toolNAreplace(toolCountryFill(mothballed),replaceby = 0)[[1]]
  # 
  # cap2015 <- cap2020[,,"Operating"] - cap2015_add[,,"post_2015_add"] + cap2015_ret[,,"post_2015_ret"] + mothballed
  # 
  # #2010 Capacity
  # cap2010_add <- additions %>% select_(~Country,`2011`:`2020`)
  # cap2010_add <- mutate_(cap2010_add,post_2010_add=rowSums(cap2010_add[,2:ncol(cap2010_add)]))
  # cap2010_add$Country <- toolCountry2isocode(cap2010_add$Country)
  # cap2010_add <- as.magpie(cap2010_add,spatial=1)
  # cap2010_add <- toolNAreplace(toolCountryFill(cap2010_add),replaceby = 0)[[1]]
  # 
  # cap2010_ret <- retirements %>% select_(~Country,`2011`:`2020`)
  # cap2010_ret <- mutate_(cap2010_ret,post_2010_ret=rowSums(cap2010_ret[,2:ncol(cap2010_ret)]))
  # cap2010_ret$Country <- toolCountry2isocode(cap2010_ret$Country)
  # cap2010_ret <- as.magpie(cap2010_ret,spatial=1)
  # cap2010_ret <- toolNAreplace(toolCountryFill(cap2010_ret),replaceby = 0)[[1]]
  # 
  # cap2010 <- cap2020[,,"Operating"] - cap2010_add[,,"post_2010_add"] + cap2010_ret[,,"post_2010_ret"] + mothballed
  # 
  # #2005 Capacity
  # cap2005_add <- additions %>% select_(~Country,`2006`:`2020`)
  # cap2005_add <- mutate_(cap2005_add,post_2005_add=rowSums(cap2005_add[,2:ncol(cap2005_add)]))
  # cap2005_add$Country <- toolCountry2isocode(cap2005_add$Country)
  # cap2005_add <- as.magpie(cap2005_add,spatial=1)
  # cap2005_add <- toolNAreplace(toolCountryFill(cap2005_add),replaceby = 0)[[1]]
  # 
  # cap2005_ret <- retirements %>% select_(~Country,`2006`:`2020`)
  # cap2005_ret <- mutate_(cap2005_ret,post_2005_ret=rowSums(cap2005_ret[,2:ncol(cap2005_ret)]))
  # cap2005_ret$Country <- toolCountry2isocode(cap2005_ret$Country)
  # cap2005_ret <- as.magpie(cap2005_ret,spatial=1)
  # cap2005_ret <- toolNAreplace(toolCountryFill(cap2005_ret),replaceby = 0)[[1]]
  # 
  # cap2005 <- cap2020[,,"Operating"] - cap2005_add[,,"post_2005_add"] + cap2005_ret[,,"post_2005_ret"] + mothballed
  # 
  # #2000 Capacity
  # cap2000_add <- additions %>% select_(~Country,`2001`:`2020`)
  # cap2000_add <- mutate_(cap2000_add,post_2000_add=rowSums(cap2000_add[,2:ncol(cap2000_add)]))
  # cap2000_add$Country <- toolCountry2isocode(cap2000_add$Country)
  # cap2000_add <- as.magpie(cap2000_add,spatial=1)
  # cap2000_add <- toolNAreplace(toolCountryFill(cap2000_add),replaceby = 0)[[1]]
  # 
  # cap2000_ret <- retirements %>% select_(~Country,`2001`:`2020`)
  # cap2000_ret <- mutate_(cap2000_ret,post_2000_ret=rowSums(cap2000_ret[,2:ncol(cap2000_ret)]))
  # cap2000_ret$Country <- toolCountry2isocode(cap2000_ret$Country)
  # cap2000_ret <- as.magpie(cap2000_ret,spatial=1)
  # cap2000_ret <- toolNAreplace(toolCountryFill(cap2000_ret),replaceby = 0)[[1]]
  # 
  # cap2000 <- cap2020[,,"Operating"] - cap2000_add[,,"post_2000_add"] + cap2000_ret[,,"post_2000_ret"] + mothballed
  # 
  # getYears(cap2020_oper) <- "y2020"
  # getYears(cap2015) <- "y2015"
  # getYears(cap2010) <- "y2010"
  # getYears(cap2005) <- "y2005"  
  # getNames(cap2020_oper) <- "Capacity.Historical"
  # getNames(cap2015) <- "Capacity.Historical"
  # getNames(cap2010) <- "Capacity.Historical"
  # getNames(cap2005) <- "Capacity.Historical"
    ...
    retAge <- retire %>% select_(~`Tracker ID`,~Country,~`Plant Age`,~`Capacity (MW)`,~Status) %>% 
      filter_(~Status=="Retired") %>% filter_(!is.na(~`Plant Age`)) %>% filter_(~`Plant Age` > 0) %>% filter_(!is.na(~`Capacity (MW)`))
    avgRetAge <- retAge %>% group_by_(~Country) %>% summarise_(Avg_Ret_Age=weighted.mean(~`Plant Age`,~`Capacity (MW)`))
    retCap <- retAge %>% group_by_(~Country) %>% summarise_(Retired_Cap=sum(~`Capacity (MW)`))
    ...
    if (subtype=="early_retire") {
      retired <- retire %>% select_(~`Tracker ID`,~Country,~`Plant Age`,~`Capacity (MW)`,~RETIRED)
      #Sum up capacity retired in each country in each year 2000-2020
      early_ret <- retired %>% filter_(!is.na(~RETIRED)) %>% group_by_(~Country,.add=TRUE) %>% group_by_(~RETIRED,.add=TRUE) %>%
        summarise_(Retired_cap=sum(~`Capacity (MW)`))
  ...
  if (subtype=="future" || subtype=="emissions") {
    ...
    retireBrown <- retire %>% select_(~`Tracker ID`,~Country,~`Planned Retire`,~`Plant Age`,~`Capacity (MW)`,~Status,~Avg_Ret_Age) %>% 
      filter_(~Status %in% c("Operating")) %>% 
      filter_(~`Planned Retire`<=2030 | ~`Plant Age` >= (~Avg_Ret_Age)) %>%
      filter_(!is.na(~`Capacity (MW)`)) %>%
      group_by_(~Country) %>% summarise_(Retiring_Cap=sum(~`Capacity (MW)`))
    ...
    retireGreen <- retire %>% select_(~`Tracker ID`,~Country,~`Planned Retire`,~`Plant Age`,~`Capacity (MW)`,~Status,~Avg_Ret_Age) %>% 
      filter_(~Status %in% c("Operating")) %>% 
      filter_(~`Planned Retire`<=2030 | ~`Plant Age` >= (~Avg_Ret_Age-5)) %>%
      filter_(!is.na(~`Capacity (MW)`)) %>%
      group_by_(~Country) %>% summarise_(Retiring_Cap=sum(~`Capacity (MW)`))
    ...
    retireNorm <- retire %>% select_(~`Tracker ID`,~Country,~`Planned Retire`,~`Plant Age`,~`Capacity (MW)`,~Status,~Avg_Ret_Age) %>%
      filter_(~Status %in% c("Operating")) %>% 
      filter_(~`Plant Age` >= 35) %>%
      filter_(!is.na(~`Capacity (MW)`)) %>%
      group_by_(~Country) %>% summarise_(Retiring_Cap=sum(~`Capacity (MW)`))
    ...
    status_2020 <- status_2020 %>% select_(~`Tracker ID`,~Country,~MW,~Jul20,~Jul19,~Jul18,~Jul17,~Jul16,~Jan15) %>% filter_(!is.na(~MW))
    ...
    emi_data <- retire %>% select_(~Country,~`Plant Age`,~`Capacity (MW)`,~Status,~`Heat rate (Btu per kWh)`,~`Emission factor (kg of CO2 per TJ)`,~`Combustion technology`,~RETIRED,~~Year) %>% 
      filter_(~Status!="Retired" & ~Status!="Cancelled" & !is.na(~`Capacity (MW)`) & is.na(~RETIRED) & !is.na(~`Emission factor (kg of CO2 per TJ)`)) %>% 
      left_join(avgRetAge,by="Country") %>% mutate_(Brown_Ret_Age=~Avg_Ret_Age+5) %>% mutate_(Norm_Ret_Age=40)
    ...
    emi_data[which(is.na(emi_data$`Plant Age`)),] <- emi_data[which(is.na(emi_data$`Plant Age`)),] %>% mutate_(`Plant Age`=~Avg_Ret_Age/2)
    ...
    emi_data <- emi_data %>% left_join(capFac,by="Country") %>% mutate_(`Plant Age`=~`Plant Age`+(~Period-2020))
    ...
    emi_data <- emi_data %>% mutate_(green_5yr_emi=ifelse(emi_data$`Plant Age`>=0 & (emi_data$`Plant Age` < emi_data$Avg_Ret_Age),
                  5 * ~`Capacity (MW)` * ~Cap_Factor * ~`Heat rate (Btu per kWh)` * ~`Emission factor (kg of CO2 per TJ)`*9.2427e-12, 0)) %>% 
      mutate_(bau_5yr_emi=ifelse(emi_data$`Plant Age`>=0 & (emi_data$`Plant Age` < emi_data$Avg_Ret_Age),
                  5 * ~`Capacity (MW)` * ~Cap_Factor * ~`Heat rate (Btu per kWh)` * ~`Emission factor (kg of CO2 per TJ)`*9.2427e-12, 0)) %>%
      mutate_(brown_5yr_emi=ifelse(emi_data$`Plant Age`>=0 & (emi_data$`Plant Age` < emi_data$Brown_Ret_Age),
                  5 * ~`Capacity (MW)` * ~Cap_Factor * ~`Heat rate (Btu per kWh)` * ~`Emission factor (kg of CO2 per TJ)`*9.2427e-12, 0)) %>% 
      mutate_(norm_5yr_emi=ifelse(emi_data$`Plant Age`>=0 & (emi_data$`Plant Age` < emi_data$Norm_Ret_Age),
                  5 * ~`Capacity (MW)` * ~Cap_Factor * ~`Heat rate (Btu per kWh)` * ~`Emission factor (kg of CO2 per TJ)`*9.2427e-12, 0))
    
    # Calculate total emissions (MtCO2) over the lifespan of plants in operation and development for each scenario
    green_emi <- emi_data %>% group_by_(~Country,.add=TRUE) %>% group_by_(~Status,.add=TRUE) %>% summarise_(green_emi = sum(~green_5yr_emi))
    
    bau_emi <- emi_data %>% group_by_(~Country,.add=TRUE) %>% group_by_(~Status,.add=TRUE) %>% summarise_(BAU_emi = sum(~bau_5yr_emi))
    
    brown_emi <- emi_data %>% group_by_(~Country,.add=TRUE) %>% group_by_(~Status,.add=TRUE) %>% summarise_(brown_emi = sum(~brown_5yr_emi))
    
    norm_emi <- emi_data %>% group_by_(~Country,.add=TRUE) %>% group_by_(~Status,.add=TRUE) %>% summarise_(norm_emi = sum(~norm_5yr_emi))
    
    
    # Calculate total emissions from all operating & pipeline plants with 40-yr lifespans
    norm_emi <- norm_emi %>% group_by_(~Country) %>% summarise_(norm_total_emi=sum(~norm_emi)/1000)
    glo_norm_emi <- norm_emi %>% summarise_(norm_global_emi=sum(~norm_total_emi))
    
    # Calculate total emissions from all operating & pipeline plants
    all_emi <- bau_emi %>% group_by_(~Country) %>% summarise_(All_total_emi=sum(~BAU_emi)/1000)
    glo_all_emi <- all_emi %>% summarise_(All_global_emi=sum(~All_total_emi))
    ...
    bau_emi <- bau_emi %>% group_by_(~Country) %>% summarize_(BAU_total_emi=sum(~BAU_emi*~Value)/1000)
    glo_bau_emi <- bau_emi %>% summarise_(Global_BAU_emi=sum(~BAU_total_emi))
    ...
    green_emi <- green_emi %>% group_by_(~Country) %>% summarize_(green_total_emi=sum(~green_emi*~Value)/1000)
    glo_green_emi <- green_emi %>% summarise_(Global_green_emi=sum(~green_total_emi))
    ...
    brown_emi <- brown_emi %>% group_by_(~Country) %>% summarize_(brown_total_emi=sum(~brown_emi*~Value)/1000)
    glo_brown_emi <- brown_emi %>% summarise_(Global_brown_emi=sum(~brown_total_emi))
    
    # Calculate capacity-weighted mean emission factor by country and plant status
    # emifac <- emi_data %>% group_by_(~Country,.add=TRUE) %>% group_by_(~Status,.add=TRUE) %>%
    #   summarise_(EF_w_mean = weighted.mean(~`Emission factor (kg of CO2 per TJ)`,~`Capacity (MW)`))
