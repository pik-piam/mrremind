...
convertRogelj2017 <- function(x, subtype) {
  if (subtype == "UNCapacity") {  # UNconditional policies
    # x <- readSource("Rogelj2017","UNCapacity",convert=FALSE)
      ...
      for (y in target_years) {
        ...
        for (r in names(final)) {
          ...
          if (!isZero(x_tmp[, , "Production-Absolute"][, , t])[r, y, ] &
              dimSums(data_combined[r, , name], na.rm = TRUE) > max(x_tmp[r, , name2])) {
...
tmp_target[6] <- tmp_target[5] - data_sel[r, , "maxprod"][, , loc + 4]
  ...
  if (subtype == "COCapacity") {
    # x <- readSource("Rogelj2017",subtype = "COCapacity",convert = FALSE)
