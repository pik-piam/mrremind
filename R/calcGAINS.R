...
calcGAINS <- function(subtype = "emission_factors", sectoral_resolution = "extended") {
  ...
  ef[r_L, 2050, "SSP1"]   <- pmin(collapseDim(ef[r_L, 2030, "SSP1"]),
                                  collapseDim(allocate_c2r_ef(ef_eclipse, r_L, select_weu, 2030, "CLE")))             # 2050: CLE30 WEU, if not higher than 2030 value
  ef[r_L, 2100, "SSP1"]   <- pmin(collapseDim(ef[r_L, 2050, "SSP1"]), collapseDim(ef_eclipse[r_L, 2030, "MFR"]))         # 2100: SLE30, if not higher than 2050 value
  # high income countries
  ef[r_HM, 2030, "SSP1"]  <- 0.75 * ef_eclipse[r_HM, 2030, "CLE"]                                                  # 2030: 75% of CLE30
  ef[r_HM, 2050, "SSP1"]  <- pmin(collapseDim(ef[r_HM,  2030, "SSP1"]), collapseDim(ef_eclipse[r_HM, 2030, "SLE"]))      # 2050: SLE30, if not higher than 2030 value
  ef[r_HM, 2100, "SSP1"]  <- pmin(collapseDim(ef[r_HM,  2050, "SSP1"]), collapseDim(ef_eclipse[r_HM, 2030, "MFR"]))      # 2100: MFR, if not higher than 2050 value
  ...
  emi[r_L, 2100, "SSP1"]   <- pmin(collapseDim(emi[r_L, 2050, "SSP1"]), collapseDim(emissions_exogenous[r_L, 2030, "MFR"]))         # 2100: SLE30, if not higher than 2050 value
  # high income countries
  ...
  ef[r_HMStrong, 2050, "SSP2"] <- pmin(collapseDim(ef[r_HMStrong,        2030, "SSP2"]),
                                       collapseDim(ef_eclipse[r_HMStrong, 2030, "SLE"]))                                # 2050: SLE30
  ef[r_HMStrong, 2100, "SSP2"] <- pmin(collapseDim(ef[r_HMStrong,        2050, "SSP2"]),
                                       collapseDim(allocate_min2r_ef(ef_eclipse, r_HMStrong, r_oecd, 2030, "SLE")))   # 2100: Lowest SLE30 or lower
  # High-Medium income countries with lower emissions goals
  ef[r_HMRest, 2030, "SSP2"]  <- ef_eclipse[r_HMRest, 2030, "CLE"]                                                   # 2030: CLE30
  ef[r_HMRest, 2050, "SSP2"]  <- pmin(collapseDim(ef[r_HMRest,       2030, "SSP2"]),
                                      collapseDim(allocate_min2r_ef(ef_eclipse, r_HMRest, r_HMRest, 2030, "CLE")))    # 2050: Min CLE30
  ef[r_HMRest, 2100, "SSP2"]  <- pmin(collapseDim(ef[r_HMRest, 2050, "SSP2"]),
                                      collapseDim(allocate_c2r_ef(ef_eclipse, r_HMRest, select_weu, 2030, "SLE")))    # 2100: SLE30 WEU
  # low income countries
  ef[r_L, 2030, "SSP2"]       <- setYears(ef_eclipse[r_L, 2020, "CLE"])                                            # 2030: CLE20
  ef[r_L, 2050, "SSP2"]       <- pmin(collapseDim(ef[r_L,       2030, "SSP2"]),
                                      collapseDim(allocate_min2r_ef(ef_eclipse, r_L, r_L, 2030, "CLE")))              # 2050: Min CLE30
  ef[r_L, 2100, "SSP2"]       <- pmin(collapseDim(ef[r_L, 2050, "SSP2"]),
  ...
  emi[r_HMStrong, 2050, "SSP2"] <- pmin(collapseDim(emi[r_HMStrong,        2030, "SSP2"]),
                                        collapseDim(emissions_exogenous[r_HMStrong, 2030, "SLE"]))                                # 2050: SLE30
  emi[r_HMStrong, 2100, "SSP2"] <- pmin(collapseDim(emi[r_HMStrong,        2050, "SSP2"]),
                                        collapseDim(emissions_exogenous[r_HMStrong, 2030, "SLE"] * 0.8))                           # 2100: Lowest SLE30 or lower -> 0.8*SLE30
  # High-Medium income countries with lower emissions goals
  emi[r_HMRest, 2030, "SSP2"]  <- emissions_exogenous[r_HMRest, 2030, "CLE"]                                                  # 2030: CLE30
  emi[r_HMRest, 2050, "SSP2"]  <- pmin(collapseDim(emi[r_HMRest,       2030, "SSP2"]),
                                       collapseDim(emissions_exogenous[r_HMRest, 2030, "SLE"]))                                  # 2050: Min CLE30 -> SLE30
  emi[r_HMRest, 2100, "SSP2"]  <- pmin(collapseDim(emi[r_HMRest, 2050, "SSP2"]),
                                       collapseDim(emissions_exogenous[r_HMRest, 2030, "SLE"] * 0.8))                              # 2100: SLE30 WEU -> 0.8*SLE30
  # low income countries
  emi[r_L, 2030, "SSP2"]       <- setYears(emissions_exogenous[r_L, 2020, "CLE"])                                           # 2030: CLE20
  emi[r_L, 2050, "SSP2"]       <- pmin(collapseDim(emi[r_L, 2030, "SSP2"]),
                                       collapseDim(emissions_exogenous[r_L, 2030, "CLE"]))                                     # 2050: Min CLE30 -> CLE30
  emi[r_L, 2100, "SSP2"]       <- pmin(collapseDim(emi[r_L, 2050, "SSP2"]),
                                       collapseDim(emissions_exogenous[r_L, 2030, "SLE"] * 0.95))                                # 2100: CLE30 WEU -> 0.95*SLE30
